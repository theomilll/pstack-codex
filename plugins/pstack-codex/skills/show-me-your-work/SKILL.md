...
At the end of the run, before handing back, check the log told the truth. Prefer the native current-task reader. If the local transcript adds missing evidence, resolve only the file ending in `-${CODEX_THREAD_ID}.jsonl` under `${CODEX_HOME:-$HOME/.codex}/sessions/` and verify its opening message. Never search another thread. Walk the log against what actually happened:
...
read-only brief. Self-review is not a substitute. Follow `../poteto-mode/references/codex-delegation.md`.
...
to specific rows or moments. If it could not run, name that verification gap. "No flags" is a valid value.
