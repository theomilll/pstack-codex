...
The check: before you keep a test, ask whether it would still pass if every function it imports returned `undefined`. If yes, inspect whether its assertions observe the intended behavior. Rewrite an assertion that does not, or delete the test if no useful assertion exists. Failing this check alone does not prove that a test covers the intended behavior.
...
**Shapes that may fail to establish the intended behavior:**
