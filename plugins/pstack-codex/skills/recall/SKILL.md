...
Transcripts live under `${CODEX_HOME:-$HOME/.codex}/sessions/`. Prefer native
task inspection first. If you must read a local transcript, match only the
exact current or named thread id, usually a filename ending in
`-${CODEX_THREAD_ID}.jsonl`, and verify the opening user message before you
trust it. Every line is one JSON record; chat messages carry `"type":"user"` or
`"type":"assistant"`.
...
3. Fan out across your chat history. Spawn parallel subagents, each taking a bounded slice of the corpus. Follow `../poteto-mode/references/codex-delegation.md`. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
