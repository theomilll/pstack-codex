...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a Codex task or session id handoff (`codex resume <id>` in the CLI), or a pushed branch you're meant to continue.
...
1. Locate the prior trail. Prefer the native Codex task reader for a task id. If missing context requires a local transcript, resolve only the exact named or current thread id under `${CODEX_HOME:-$HOME/.codex}/sessions/` and verify its opening message. A pushed branch is also a valid trail. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
