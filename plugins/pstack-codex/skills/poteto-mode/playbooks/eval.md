...
4. **Spawn N parallel candidates** on `gpt-5.6-sol` per the **arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on `gpt-5.6-sol` in a fresh context per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a task label.
