...
- The judge can know it's judging but sees outputs by sanitized label only, never by candidate identity or prior scores.
...
4. **Spawn N parallel candidates** per the **arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** in a fresh context per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a task label.
6. **Verify the chain from task evidence, not self-report.** Read each candidate through the native Codex task reader. If an exact candidate task id has a local transcript and the task reader omits needed tool evidence, resolve only that id under `${CODEX_HOME:-$HOME/.codex}/sessions/`. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
7. **Read every candidate output yourself** end to end. Compare to the judge's verdict. Disagreement calls for checking the evidence and clarifying the rubric. Synthesize.
