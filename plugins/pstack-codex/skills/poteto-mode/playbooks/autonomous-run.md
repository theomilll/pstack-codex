...
2. Pick a Codex-native wake mechanism. In the app, use a heartbeat automation. For a bounded local run, use a watcher task or terminal session that rechecks the predicate. An event to watch (CI, a merge, a ref advancing) gets an event watcher with a time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
