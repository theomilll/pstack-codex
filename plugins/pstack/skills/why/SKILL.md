...
Before spawning investigators, list the available MCP servers. MCP tools are
named `mcp__<server>__<tool>`. Enumerate them with the host's tool search,
usually by querying `mcp__`.
