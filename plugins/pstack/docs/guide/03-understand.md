...
[`$pstack:recall`](../../skills/recall/SKILL.md) mines your own recent task history plus the shared record (issues, prior fixes, errors still firing) and hands back a brief on where things stand and what's next. Use it when you're returning to a topic cold. If you want to resume one specific task, that's the Session pickup playbook below, not `$pstack:recall`.
