...
- Codex's native wake mechanism, a heartbeat automation in the app or a bounded watcher task in CLI flows, re-checks the finish condition on events or a heartbeat. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) owns that wiring.
...
[Orchestrate](../../skills/poteto-mode/playbooks/orchestrate.md) is for a program that outlives any single task: multi-day, many stacked PRs, fleets of subagents under one standing coordinator task. The coordinator authors briefs, collects what its subagents finish, keeps the lowest unmerged PR green, and never writes code itself. It's deliberately heavy machinery. If one task could finish the work in a session, the playbook itself routes you back to the overnight contract above:
...
**Pitfall:** a duration is not a finish condition. "work on this for 4 hours" gives the agent nothing to check, and you'll wake up to four hours of motion instead of a result. Give the wake loop a predicate that can pass or fail.
