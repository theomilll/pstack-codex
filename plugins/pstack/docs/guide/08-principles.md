...
pstack ships 23 principles as individual skills. `$pstack:poteto-mode` applies the ones the task triggers and names the principles whose leaf instructions it actually read, along with the decision each changed.
...
- [Test Behavior, Not Implementation](../../skills/principle-test-behavior-not-implementation/SKILL.md) calls the code the way its users do and checks observable results. It rewrites or removes assertions that do not establish the intended behavior.
